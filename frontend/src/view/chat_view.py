...
class ChatView(AbstractStrategyView):
    ...
    def __init__(
        ...
        self._session_state_service.initialize_openai_api_key_handler()
    ...
    def _on_chat_submit(_self, prompt: str) -> None:
        ...
        message: str = _self._http_client_service.get_chat_response(st.session_state.messages, _self._api_key)
    ...
    def _handle_api_key(self) -> None:
        if not self._api_key:
            ...
            if not st.session_state["api_key_set"]:
                ...
                if self._api_key:
                    self._session_state_service.set_openai_api_key(self._api_key)
                    st.rerun()
                else:
                    st.stop()
                    
            else:
                self._api_key = self._session_state_service.get_openai_api_key()
